...
##
# @class Board
# @brief Responsible for drawing the Tetris board grid and border.
#
class Board:

    ##
    # @brief Draws the board grid and the border rectangle.
...
##
# @brief Draws all UI elements: score box, level, lines, next/hold boxes, icons.
#
# @param screen The pygame display surface.
# @param font_score Font used for score text.
# @param font_label Font for UI labels.
# @param font_value Font for numeric values.
...
def draw_ui(screen, font_score, font_label, font_value, score, level, lines):
    ...
    next_box_rect = pygame.Rect(next_box_x + 17, values_y - 10, next_box_width, 40)
    ...
    hold_box_rect = pygame.Rect(hold_box_x, values_y - 10, hold_box_width, 40)
    pygame.draw.rect(screen, WHITE, hold_box_rect, 1, border_radius=5)


##
# @brief Main game loop initializer. Creates window and draws board + UI.
#
def main():
    pygame.init()
    pygame.font.init()
